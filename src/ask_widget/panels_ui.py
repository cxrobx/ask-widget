...
.hist-list{display:flex;flex-direction:column;gap:1px} .hist-row{display:flex;flex-direction:column;gap:2px;width:100%;padding:8px 10px;border:0;border-radius:8px;background:transparent;color:inherit;text-align:left;transition:background-color 75ms}
...
.hist-block pre{max-height:240px;margin:0;padding:11px 12px;overflow:auto;border:1px solid var(--line-soft);border-radius:8px;background:rgb(var(--ink)/.035);color:rgb(var(--ink));font:12.5px/1.55 -apple-system,BlinkMacSystemFont,"SF Pro Text",sans-serif;white-space:pre-wrap}
...
<div class=field><label for=appearance-theme>Color theme</label><select id=appearance-theme name=appearance_theme><option value=system>System</option><option value=light>Light</option><option value=dark>Dark</option></select></div>
...
<div class="field wide"><label class=check><input name=markdown_follow_obsidian type=checkbox> Match vault Markdown appearance</label><p id=markdown-theme-status class=field-help>Uses reading styles shared by the Onyx Obsidian plugin. Applies to Markdown notes only.</p></div>
<div class="field wide"><label class=check><input name=sidebar_follow_obsidian type=checkbox> Match vault sidebar appearance</label><p id=sidebar-theme-status class=field-help>Uses your Obsidian file explorer’s look, shared by the Onyx Obsidian plugin.</p></div></form></section>
...
function applyTheme(raw){const theme=['light','dark'].includes(raw)?raw:'system';document.documentElement.dataset.theme=theme;$('#appearance-theme').value=theme;const h=native&&window.webkit.messageHandlers.askwAppearance;if(h)Promise.resolve(h.postMessage({theme})).catch(()=>{});paintGlass()}
...
if('vault_root' in patch||'html_vault_root' in patch){refreshRoots();reloadTrees()}if('sidebar_follow_obsidian' in patch){sidebarStatus();syncSidebarTheme(true)}if('markdown_follow_obsidian' in patch)markdownStatus();
